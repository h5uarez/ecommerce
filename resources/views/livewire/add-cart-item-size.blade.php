<div x-data>
    <div>
        ...
        <select dusk="size" wire:model="size_id" class="form-control w-full">
            ...
            @foreach ($sizes as $size)
                <option dusk="sizetype" value="{{ $size->id }}">{{ $size->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="mt-2">
        ...
        <select dusk="color" wire:model="color_id" class="form-control w-full">
            ...
            @foreach ($colors as $color)
                <option dusk="colortype" value="{{ $color->id }}">{{ __(ucfirst($color->name)) }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="flex">
        ...
        <div class="flex-1">
            <x-button dusk="add-full-item" x-bind:disabled="$wire.qty > $wire.quantity" wire:click="addItem"
