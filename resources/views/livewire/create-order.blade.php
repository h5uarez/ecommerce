    ...
    <div class="col-span-3">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="mb-4">
                ...
                <x-jet-input dusk="contactname" type="text" wire:model.defer="contact"
            ...
            <div>
                ...
                <x-jet-input dusk="contactnumber" type="text" wire:model.defer="phone"
                    placeholder="Introduzca el teléfono de contacto" class="w-full" />
        ...
        <div x-data="{ envio_type: @entangle('envio_type') }">
            ...
            <div class="bg-white rounded-lg shadow">
                <label class="px-6 py-4 flex items-center">
                    <input dusk="domicilio" x-model="envio_type" type="radio" name="envio_type" value="2"
                        class="text-gray-600">
                    <span class="ml-2 text-gray-700">Envío a domicilio</span>
                </label>
                <div dusk="addressform" class="px-6 pb-6 grid grid-cols-2 gap-6" :class="{ 'hidden': envio_type != 2 }">
                    <div>
                        <x-jet-label value="Departamento" />
                        <select dusk="selectdepartment" class="form-control w-full" wire:model="department_id">
                            <option value="" disabled selected>Seleccione un departamento
                            </option>
                            @foreach ($departments as $department)
                                <option dusk="optiondepartment" value="{{ $department->id }}">
                    ...
                    <div>
                        ...
                        <select dusk="selectcity" class="form-control w-full" wire:model="city_id">
                            <option value="" disabled selected>Seleccione una ciudad</option>
                            @foreach ($cities as $city)
                                <option dusk="optioncity" value="{{ $city->id }}">{{ $city->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select dusk="selectdistrict" class="form-control w-full" wire:model="district_id">
                            <option value="" disabled selected>Seleccione un distrito</option>
                            @foreach ($districts as $district)
                                <option dusk="optiondistrict" value="{{ $district->id }}">{{ $district->name }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div>
            <x-jet-button dusk="buybutton" wire:loading.attr="disabled" wire:target="create_order"
